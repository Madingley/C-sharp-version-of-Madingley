...
nc<-create.nc("G:/WCMC Work/Environmental Data/Climate/monthly/1_degree/MeanTemp.nc")
...
var.def.nc(nc,"Temperature","NC_DOUBLE",c("Lat","Lon","Month"))

# Add the missing-data attribute to the temperature variable in the netCDF
att.put.nc(nc,"Temperature","missing_value","NC_DOUBLE",-9999.0)
...
for (f in 1:12)
{
  ...
  fname<-paste("G:/WCMC Work/Environmental Data/Climate/monthly/1_degree/tmean/tmean_",f,sep="")
  # Read the file
  map<-readGDAL(fname)
  
  # Convert to degrees Celsius (from degrees x 10)
  map$band1<-(map$band1/10)
  ...
  var.put.nc(nc,"Temperature",monthData,c(1,1,f),c(length(lats),length(lons),1))
}
